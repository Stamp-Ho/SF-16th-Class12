...
export async function createNewSeatRound(
  ...
  if (drawError || !draw) {
    throw new Error("선택한 추첨 결과를 불러올 수 없습니다.");
  }

  const drawList = draw.result_data as { order: number; name: string }[];
  ...
  for (let i = 0; i < drawList.length; i += 2) {
    const groupNum = Math.floor(i / 2) + 1;
    const m1 = drawList[i]?.name || "";
    const m2 = drawList[i + 1]?.name || "";
